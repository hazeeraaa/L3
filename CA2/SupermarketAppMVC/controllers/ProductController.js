...
const ProductController = {
	...
	userOrders(req, res) {
		...
		Order.getOrdersByUser(userId, async function(err, orders) {
			if (err) {
				console.error('Failed to fetch user orders:', err);
				orders = [];
			}
			// attach latest refund info to each order
			try {
				const Refund = require('../models/Refund');
				await Promise.all((orders || []).map(order => new Promise((resolve) => {
					Refund.getLatestByOrderId(order.id, function(rErr, r) {
						order.refund = r || null;
						resolve();
					});
				})));
			} catch (attachErr) {
				console.error('Failed to attach refund info to orders:', attachErr);
			}
	...
	viewInvoice(req, res) {
		...
		Order.getOrderById(orderId, function(err, order) {
			...
			// attach latest refund info
			const Refund = require('../models/Refund');
			Refund.getLatestByOrderId(order.id, function(rErr, refund) {
				if (rErr) console.error('Failed to load refund for invoice:', rErr);
				order.refund = refund || null;
				// If the invoice was reached immediately after a payment flow, an optional query param
				// `paid` (e.g. ?paid=paypal) may be present — pass that to the template so it can
				// display a friendly success message.
				const paymentSuccess = req.query && req.query.paid ? true : false;
				const paymentMessage = paymentSuccess ? 'Payment completed successfully.' : null;
				res.render('invoice', { order, user, paymentSuccess, paymentMessage });
			});
		});
	},
	...
	async pay(req, res, capture) {
		try {
			...
			// transaction persistence deferred until after local order creation
	...
	// delegate payment handling to PaymentController
	pay(req, res, capture) {
		return require('./PaymentController').pay(req, res, capture);
	},
	...
	// delegate NETS finalization to PaymentController
	netsComplete(req, res) {
		return require('./PaymentController').netsComplete(req, res);
	},
	...
	adminOrders(req, res) {
		Order.getAllOrders(function(err, orders) {
			...
			const Refund = require('../models/Refund');
			// attach latest refund info for each order and fetch pending set
			Refund.getPending(function(rErr, pending) {
				if (rErr) {
					console.error('Failed to fetch pending refunds for admin orders:', rErr);
					pending = [];
				}
				const pendingSet = new Set((pending || []).map(p => Number(p.order_id)));
				// attach latest refund record to each order
				Promise.all((orders || []).map(order => new Promise(resolve => {
					Refund.getLatestByOrderId(order.id, function(fErr, f) {
						order.refund = f || null;
						resolve();
					});
				}))).then(() => {
					res.render('adminOrders', { orders, user: req.session.user, pendingRefundOrders: pendingSet });
				}).catch(e => {
					console.error('Failed to attach refunds to orders for admin view:', e);
					res.render('adminOrders', { orders, user: req.session.user, pendingRefundOrders: pendingSet });
				});
			});
		});
	...
	requestRefund(req, res) {
		return require('./PaymentController').requestRefund(req, res);
	},
	...
	adminRefunds(req, res) {
		const Refund = require('../models/Refund');
		Refund.getPending(function(err, rows) {
			if (err) {
				console.error('Failed to fetch pending refunds:', err);
				rows = [];
			}
			res.render('adminRefunds', { refunds: rows, user: req.session.user });
		});
	},
	...
	markPickupCollected(req, res) {
		...
		Order.updatePickupCollected(orderId, 1, function(err, info) {
			...
			// Also mark the order status as completed so the user sees it as completed
			Order.updateStatus(orderId, 'completed', function(sErr) {
				if (sErr) console.error('Failed to update order status after pickup collected:', sErr);
	...
	// Admin: refund via PayPal for a given order (delegates to PaymentController)
	refundPayPal(req, res) {
		return require('./PaymentController').refundPayPal(req, res);
	},

	// Admin: approve a refund request (delegate)
	approveRefund(req, res) {
		return require('./PaymentController').approveRefund(req, res);
	},

	// Admin: reject a refund request (delegate)
	rejectRefund(req, res) {
		return require('./PaymentController').rejectRefund(req, res);
	},
