...
app.post('/api/paypal/capture-order', async (req, res) => {
    try {
        ...
        if (status === 'COMPLETED' || status === 'COMPLETED') {
            // delegate to PaymentController.pay to finalize order locally
            return require('./controllers/PaymentController').pay(req, res, capture);
        }
...
app.post('/nets-qr/complete', async (req, res) => {
    // delegate to PaymentController.netsComplete to create order and clear cart
    return require('./controllers/PaymentController').netsComplete(req, res);
});
...
// NETS QR success page (compat with NETSDemo)
app.get('/nets-qr/success', (req, res) => {
    // Accept an invoice query param (e.g. /nets-qr/success?invoice=/invoice/123)
    const invoiceUrl = req.query && req.query.invoice ? req.query.invoice : null;
    res.render('netsTxnSuccessStatus', { message: 'Transaction Successful!', invoiceUrl });
});
        ...
        try {
            ...
            const response = await axios.post(
                ...
                { txn_retrieval_ref: txnRetrievalRef, frontend_timeout_status: frontendTimeoutStatus },
                {
                    ...
                    // Add a reasonable timeout so SSE doesn't hang indefinitely
                    timeout: 10000
                }
            );
            ...
            // Normalize response_code/txn_status checks to handle strings and numbers
            const isSuccess = resData && (resData.response_code == "00" || resData.response_code == 0 || String(resData.response_code) === "00") && (resData.txn_status == 1 || String(resData.txn_status) === '1');
            ...
            if (isSuccess) {
                ...
                res.end();
            } else if (frontendTimeoutStatus == 1 && resData && ((resData.response_code != "00" && resData.response_code != 0) || resData.txn_status == 2 || String(resData.txn_status) === '2')) {
...
app.post('/orders/:orderId/request-refund', requireLogin, (req, res) => ProductController.requestRefund(req, res));
...
app.post('/admin/orders/:id/refund', requireAdmin, (req, res) => ProductController.refundPayPal(req, res));

// Admin: refund requests list and actions
app.get('/admin/refunds', requireAdmin, (req, res) => ProductController.adminRefunds(req, res));
app.post('/admin/refunds/:id/approve', requireAdmin, (req, res) => ProductController.approveRefund(req, res));
app.post('/admin/refunds/:id/reject', requireAdmin, (req, res) => ProductController.rejectRefund(req, res));
...
// Ensure refunds table exists at startup to avoid runtime errors when creating requests
function ensureRefundsTable() {
        const sql = `CREATE TABLE IF NOT EXISTS refunds (
        ...
        db.query(sql, function(err) {
                if (err) return console.error('Failed to ensure refunds table exists:', err);
